...
- Minimal near-term goal: the #1 current `RunawayCandidates` row should
...
- Priority #1: names in today's summary `RunawayCandidates` should be in tomorrow's `research_top_gainers.csv`.
...
- `RunawayCandidates`: above-mid / runaway / continuation style ideas.
...
- If the ticker was below the daily Bollinger mid on the last closed daily bar, it belongs to `ReversalCandidates`.
- If the ticker was at or above the daily Bollinger mid on the last closed daily bar, it belongs to `RunawayCandidates`.
...
- `RunawayCandidates` should be the strongest next-day research proxy.
...
- Use `research_top_gainers.csv` as the main `RunawayCandidates` template source.
...
- `ReadyNow`: H4 real Bollinger trigger confirms the launch. The upper band
  ...
  be promoted to `RunawayCandidates`.
...
ready for immediate `RunawayCandidates` promotion, while SHLS-like
